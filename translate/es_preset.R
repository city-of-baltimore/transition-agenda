# Produce content and functions for ui and server
  ...
  # Fonts, Colors, and Icons 
    ...
    ggpalette1 <- cbind(
      ...
      c("Completar","Todavía no empezado","En curso")
    )
    ...
    symbolColor <- function(status) {
      return(ifelse(tolower(status) == "en curso", 
        bc_gold,
        ifelse(tolower(status) == "completar",
          bchd_blue,
          "#DCDCDC")))
    }
  ...
  #Load and format data
    ...
    tbPriorities <- pg1 %>%
      mutate(Progress = factor(Progress, levels=c("Completar", "En curso", "Todavía no empezado"))) %>% 
      select(-c(2:5,11:22)) %>%
      arrange(Sp_Committee, Sp_Status) %>%
      mutate(.,"Icon" = with(.,case_when(
                           (Sp_Committee == "Edificio de Seguridad Pública") ~ safety,
                           (Sp_Committee == "Haciendo Baltimore equitativa") ~ equity1,
                           (Sp_Committee == "Priorización de nuestra juventud") ~ education,
                           (Sp_Committee == "Fomento de la confianza pública") ~ governance,
                           (Sp_Committee == "COVID-19 Recuperación") ~ health,
                           T ~ as.character(finance)
                                  )))
    
    tbActionsNested <- tbPriorities %>% 
      mutate(ActionProgressParties = mapply(c, Sp_Action, 
                                            symbol(Sp_Status),
                                            Sp_Agencies, 
                                            SIMPLIFY = F)) %>% 
      select(c("Sp_Committee", "ActionProgressParties")) %>% 
      group_by(Sp_Committee) %>%
      summarise(ActionProgressParties = list(unique(ActionProgressParties))) 
    
    
    actionsTotal <- nrow(tbPriorities)
    actionsComplete <- nrow(tbPriorities[ which(tbPriorities$Sp_Status == "Completar"),])
    actionsInProgress <- nrow(tbPriorities[ which(tbPriorities$Sp_Status == "En curso"),])
    actionsRemaining <- actionsTotal - actionsComplete - actionsInProgress
    
    tbCommittees <- pg3 %>% 
      rename("Priority Area" = Es_Committee) %>% 
      left_join(tbActionsNested, by = c("Priority Area" = "Sp_Committee")) %>%
      mutate(.,"Icon" = with(.,case_when(
        (`Priority Area` == "Edificio de Seguridad Pública") ~ safety,
        (`Priority Area` == "Haciendo Baltimore equitativa") ~ equity1,
        (`Priority Area` == "Priorización de nuestra juventud") ~ education,
        (`Priority Area` == "Fomento de la confianza pública") ~ governance,
        (`Priority Area` == "COVID-19 Recuperación") ~ health,
        T ~ as.character(finance)
      ))) %>% 
      mutate(Progress = lapply(ActionProgressParties, FUN=priorityAreaProgressBar)) %>%
      select(-c(1:3,5:7)) %>% 
      mutate(Name = `Priority Area`) %>%
      select(c(1,5,2,3,4)) %>%
